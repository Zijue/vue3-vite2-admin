<template>
  <el-breadcrumb separator="/">
    <el-breadcrumb-item v-for="(item, index) in levelList" :key="item.path">
      ...
      <a v-else @click.prevent="handleLink(item as Required<typeof item>)">{{
        item.meta?.title
      }}</a>
    </el-breadcrumb-item>
  </el-breadcrumb>
...
export default defineComponent({
  ...
  setup() {
    ...
    const levelList = ref<PartialRouteLocationMatched[]>([]) // 导航列表，存放matched里筛选的路由记录
    ...
    const getBreadcrumb = () => {
      ...
      // route.meta.breadcrumb自定义属性，如果为false匹配到时，将不会在面包屑导航显示该路由
      /**
       {
           path: 'menu',
           meta: {
    ...
    const pathCompile = (path: string) => {
      // 根据路径编译正则函数，并接受具体参数；比如根据正则/user/:id，帮你将:id替换成具体路径
      const toPath = compile(path) // 比如path /user/:id
