      ...
      >
        <el-menu-item :index="resolvePath(theOnlyOneChildRoute.path)">
          <el-icon
            ...
            class="no-inherit"
          >
            <component :is="icon.slice(8)" />
          </el-icon>
          <svg-icon v-else-if="icon" :icon-name="icon" class-name="menu-icon" />
          <template #title>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // 渲染菜单主要先看子路由
    // 比如我们的路由，一级路由一般都是layout组件，耳机路由才是我们需要考虑要渲染成菜单的
    ...
    const icon = computed(() => {
      ...
      return (theOnlyOneChildRoute.value?.meta?.icon ||
        (props.item.meta && props.item.meta.icon)) as string | null
    })
    ...
    // 设置alwaysShow: true，这样它就会忽略上面定义的规则，一直显示根路由
